...
| P0-15 | Closed (new, was a live incident) | `vehicle-vitals-prod` had Firebase App Check enforcement **ENFORCED** on Identity Toolkit (since 2026-07-10), Firestore, and Storage (both since 2026-06-03), while neither the web nor mobile client has any App Check integration at all (no `firebase_app_check` package, no init code anywhere). Every real user hitting production has been unable to sign in since July 10, and unable to read/write Firestore or Storage since June 3. Discovered when a HADES device build (mistakenly defaulting to the `production` Firebase environment) failed sign-in with `FIRAuthErrorDomain code 17999`, `"Firebase App Check token is invalid"`. | Disabled enforcement (`UNENFORCED`) on all 3 services in `vehicle-vitals-prod` via the App Check API, restoring prior working behavior immediately (confirmed via `GET .../services` showing all 3 back to `UNENFORCED`). This is a mitigation, not a fix — App Check itself is still not implemented on any client. Proper fix (App Check client integration + re-enable enforcement) is a separate, not-yet-scoped follow-up; do not re-enable enforcement on any of these 3 services until that ships and is verified, or every real user will hit this outage again. |
